	...
	function checkIsPaybillInProcess(billNumber,monthName,yearName) ///CheckPaybill/{billNumber}/{monthName}/{yearName}
	{
		flag=0;
		 $.ajax({
		      type: "GET",
		      url: "../master/CheckPaybillForRop/"+billNumber+"/"+monthName +"/"+yearName,
		      async: false,
		      dataType : 'json',
...
//		    alert(data);
	...
	function checktheEntryForRop(yearId,monthId,billGroup) ///CheckPaybill/{billNumber}/{monthName}/{yearName}
	{
		flag=0;
		 $.ajax({
		      type: "GET",
		      url: "../master/checktheEntryForRop/"+yearId+"/"+monthId +"/"+billGroup,
		      async: false,
		      dataType : 'json',
		    // contentType:'application/json',
		      error: function(data){
 	 	 ...
 	 	 if(month!="0" && year!="0" && billGroup!="0"){
 	 		...
 	 		if(ispaybillinProcess == 0)
 	 			...
 	 			e.preventDefault();
 	 				...
 	 				{
 	 				var isdatapresent = checktheEntryForRop(year,month,billGroup);
 	 	 	 		if(isdatapresent == 0)
 	 	 	 			{
