...
function SearchEmployee(e) {
	...
	var flag=true;
		$.ajax({
			type : "GET",
			url : "../level1/isEmpMappedWithBillGroup/"+searchName,
			async : false,
...
//				dataFromServer=data;
		...
		$.ajax({
			type : "GET",
			url : "../level1/getSevaarthIdMappedWithPaybill/"+sevaarthid+"/"+monthId+"/"+yearId,
			async : false,
			contentType: "application/json",
	        dataType: "json",
			error : function(data) {
...
function calculateSalaryBasedonBasic(rownumber){
	...
	if(toDate!="" && fromDate!=""){
		...
		.ajax({
			type : "GET",
			url : "calculateDAArrears/" + sevaarthid + "/"+empId+"/"+month+"/"+year+"/"+noOfDays+"/"+paybillMonth+"/"+paybillYear+"/"+basicForCalculation,
			async : true,
			contentType : 'application/json',
			error : function(data) {
...
function loadSalaryFromRuleEngine(rownumber) {
	...
	if(toDate!="" && fromDate!=""){
		...
		.ajax({
			type : "GET",
			url : "calculateDAArrears/" + sevaarthid + "/"+empId+"/"+month+"/"+year+"/"+noOfDays+"/"+paybillMonth+"/"+paybillYear,
			async : true,
			contentType : 'application/json',
			error : function(data) {
				swal("Salary not calculated from Rule engine becuase of some error");
...
//	var basicPay = document.getElementById("txtBasicPay"+counter).value;
	...
	var netPay ='';
	if( Number(totalValueOfAllowances)!=0 ||  Number(totalValueOfAllowances) !='0'){
		
		 netPay = Number(0) + Number(totalValueOfAllowances) - Number(totalValueOfDeductions);
	}else{
		 netPay = Number(0) +  Number(totalValueOfDeductions);
	}
...
//	alert("totalAllowances="+totalAllowances);
	...
	var GrossAmount = Number(0) + Number(totalValueOfAllowances);
...
//			+ '&nbsp;&nbsp;&nbsp;<img src="images/CalendarImages/ico-calendar.gif" onClick="window_open(\'txtFromDate'+nextRow+'\', 375, 570, \'\', \'\', '+nextRow+');"	style="cursor: pointer;"/>' ;
	...
	Cell31.innerHTML='<select name="basicForCalculation" class="form-control" id="basicForCalculation'+nextRow+'"   id="basicForCalculation'+nextRow+'"     onchange="calculateSalaryBasedonBasic('+nextRow+');"  ><option value="newBasic">New Basic</option><option value="oldBasic">Old Basic</option></select>';
	*/
	
	
	
	var basicPayRow='<input type="hidden" size="8" name="txtBasicPay" id="txtBasicPay'+nextRow+'" class="form-control"  value="0" readonly  />';
	var basicForCalculation='<input type="hidden" name="basicForCalculation" class="form-control" id="basicForCalculation'+nextRow+'"   value="newBasic" readonly  />';
	
	
	var Cell3 = newRow.insertCell(-1);
	Cell3.style.border ="1px solid rgb(255, 218, 178)";
	Cell3.innerHTML = '<input type="text" size="8" name="txtNoOfDays" id="txtNoOfDays'+nextRow+'"  class="form-control"  value="" readonly="readonly" />'+basicPayRow+basicForCalculation;
...
//		          } );
	...
	.ajax({
		type : "GET",
		url : "saveBrokenPeriodDAArrayPay/" + sevaarthid + "/"+empId+"/"+fromDates+"/"+toDates+"/"+noOfDays+"/"+paybillMonth+"/"+paybillYear+"/"+basicPays+"/"+allowancesCodes+"/"+allowancesValues+"/"+netPays+"/"+reasons+"/"+remarks+"/"+basicForCalculation,
		async : true,
		contentType : 'application/json',
		error : function(data) {
...
//			var len = data.length;
		...
		// location.reload();
		 ...
		 location.href="../level1/brokenPeriodArrears";
