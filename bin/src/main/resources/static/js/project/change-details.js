jQuery(document).ready(function($) {
	$("#pfdescription").prop("disabled",false);
	...
	//add custome method for validation
...
//					min:8,
				...
				cityClass : {
					required :function() {
						//returns true if uid3 is empty
						return (parseInt($('#cityClass').val()) !=0);
					},
				},
...
//			dtInitialAppointmentParentInst : "Please Select Initial Joining Date",
		    ...
		    reasonForChangedtls : " Please enter remark",
...
function forwardToDDo() {
	...
	var reasonForChangedtls = $("#remark").val();
	if (reasonForChangedtls == "") {
						...
						success : function(data) {
							...
							if (checkFlag == 0) {
								...
								swal('Entered Email ID: '
										+ email
										+ ' is already present in system. Please enter correct Email ID.');
	...
	if (uniqid != '') {
		$.ajax({
			...
			success : function(data) {
				...
				// alert(checkFlag);
				swal({ 
					 text: "Approved Successfully Sevaarthid is generated "+ sevaarthIdres,
...
function readURL1image(input) {
	if (input.files && input.files[0]) {
		...
		if (filekbsize >= 250) {
			input.value = '';
			swal("File too Big, please select a file less than 250kb");
			return false;
		}
...
function readURL1imagesign(input) {
	if (input.files && input.files[0]) {
		...
		if (filekbsize >= 150) {
			...
			swal("File too Big, please select a file less than 150kb");
			return false;
		}
		var reader = new FileReader();
