/*	$("#rptBtn_GenerateReport").click(function(){
	...
	*/
	...
	function checktheEntryForBlwf(billNumber,monthName,yearName) ///CheckPaybill/{billNumber}/{monthName}/{yearName}
	{
		flag=0;
		 $.ajax({
		      type: "GET",
		      url: "../master/checktheEntryForBlwf/"+billNumber+"/"+monthName +"/"+yearName,
		      async: false,
		      dataType : 'json',
		    // contentType:'application/json',
		      error: function(data){
 	 	 ...
 	 	 if(month!="0" && year!="0" && billGroup!="0"){
 	 		...
 	 		if(ispaybillinProcess == 0)
 	 			...
 	 			e.preventDefault();
 	 				...
 	 				{
 	 				var isdatapresent = checktheEntryForBlwf(billGroup,month,year);
 	 	 	 		if(isdatapresent == 0)
 	 	 	 			{
