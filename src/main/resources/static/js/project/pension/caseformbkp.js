...
$("#isSuperAnnToFamily").change(function() {
	var chk=$("#isSuperAnnToFamily").prop("checked");
...
function newPensionCalculation() {
	...
	var gratuityAmt=0;
		...
		if (ClassOfPnsn == 'Family Pension' && Number(totalSms)>0  && ($("input[name='_isSuperAnnToFamily']").val() !='true') ) {
...
$("#txtDateOfExpiry").change(function() {
	...
	if($("input[name='_isSuperAnnToFamily']").val() !='true'){
		setFP1AndFp2Date(2);
...
$("#btnAddRow1").click(function(){
		var index=$('#daArrearTable >tbody >tr').length;
		var srNo=$('#daArrearTable >tbody >tr').length+1;
...
$(document).on('click', '.delete', function(){ 
	...
	$("#daArrearTable").on('blur',".fromDate,.toDate",function(){
		...
		var fromDate = row.find('.fromDate').val();
		var toDate = row.find('.toDate').val();
		var calculateArerarOn = $('#txtTotlPnsnAmt').val();
			 ...
			 row.find('.remarks').val(remark);
				...
				$.ajax({
					...
					data: {
				        ...
				        enhFamPenDate:txtFP1Date,
				        famPenDate:txtFP2Date,
				        enhFamPenAmt:txtFP1TotlAmt,
				        famPenAmt:txtFP2TotlAmt,
				   },
					success : function(data) {
						 console.log(data);
						var len = data.length;
