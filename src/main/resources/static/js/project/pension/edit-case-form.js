   			...
   			if (caseRejectRemark == "" ) {
   			   ...
   			   swal({
   				   ...
   				   if (willDelete) {
   					   $.ajax({
   						   type: "GET",
   						   url: context+"/pension/rejectCaseForm/"+ppoNo+ "/" + caseRejectRemark,
   						   async: true,
   						   error: function(data){
   							   console.log(data);
   							   swal("Something went wrong !", {
	   ...
	   if(isTrue==true){
		   swal({
			   ...
			   if (willDelete) {
				   $.ajax({
					   type: "GET",
					   url: context+"/pension/deleteRecoveryetail/"+penEmpRecId,
					   async: true,
					   error: function(data){
						   console.log(data);
						   swal("Something went wrong !", {
