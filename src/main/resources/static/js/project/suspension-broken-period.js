...
function calculateSalaryBasedonBasic(rownumber){
	...
	var basicForCalculation=document.getElementById("basicForCalculation"+rownumber).value;
	...
	if(toDate!="" && fromDate!=""){
		...
		.ajax({
			type : "GET",
			url : "calculateEmployeeSalaryBasedofBasic/" + sevaarthid + "/"+empId+"/"+month+"/"+year+"/"+noOfDays+"/"+paybillMonth+"/"+paybillYear+"/"+basicForCalculation+"/"+fromDate+"/"+toDate,
			async : true,
			contentType : 'application/json',
			error : function(data) {
...
function calculateSuspensionEmployeeSalaryBasedofBasic(rownumber) {
	var url = "";
	var toDate=document.getElementById("txtToDate"+rownumber).value;
	var fromDate=document.getElementById("txtFromDate"+rownumber).value;
	var paybillYear=  document.getElementById("yearName").value;
	...
	if(toDate!="" && fromDate!=""){
		...
		/////var checkPaybillAlreadyExists = checkRegPaybillAlreadyExistsforDate(toDate,fromDate,sevaarthid,empId);
		///if(checkPaybillAlreadyExists == true){
		
		var checkDate = checkEmpSuspensionDate(toDate,fromDate,sevaarthid,empId);
		if(checkDate == true){
			
		$
		.ajax({
			type : "GET",
			url : "calculateSuspensionEmployeeSalaryBasedofBasic/" + sevaarthid + "/"+empId+"/"+month+"/"+year+"/"+noOfDays+"/"+paybillMonth+"/"+paybillYear+"/"+fromDate+"/"+toDate,
			async : true,
			contentType : 'application/json',
			error : function(data) {
				swal("Salary not calculated from Rule engine becuase of some error");
			...
			document.getElementById("txtNoOfDays"+rownumber).value="";
			swal("Please enter correct From Date");
			document.getElementById("txtFromDate"+rownumber).value="";
			document.getElementById("txtToDate"+rownumber).value="";
		}
		/*}else{
			document.getElementById("txtNoOfDays"+rownumber).value="";
			swal("Paybill is already Generated");
			document.getElementById("txtFromDate"+rownumber).value="";
			document.getElementById("txtToDate"+rownumber).value="";
		}*/
...
//			+ '&nbsp;&nbsp;&nbsp;<img src="images/CalendarImages/ico-calendar.gif" onClick="window_open(\'txtFromDate'+nextRow+'\', 375, 570, \'\', \'\', '+nextRow+');"	style="cursor: pointer;"/>' ;
	...
	Cell2.innerHTML = '<input type="date" size="10" name="txtToDate" class="form-control"  id="txtToDate'+nextRow+'" maxlength="10" onkeypress="digitFormat(this);dateFormat(this);" 	onBlur="checkDateForThisMonth(this,'+nextRow+');checkSameMonth(this,'+nextRow+');setNoOfDays('+nextRow+');calculateSuspensionEmployeeSalaryBasedofBasic('+nextRow+')" value="" />' ;
	...
	var Cell31 = newRow.insertCell(-1);
	Cell31.style.border ="1px solid rgb(255, 218, 178)";
	Cell31.innerHTML='<select name="basicForCalculation" class="form-control" id="basicForCalculation'+nextRow+'"   id="basicForCalculation'+nextRow+'" ><option value="newBasic">New Basic</option><option value="oldBasic">Old Basic</option></select>';
...
//	var totalRows = 1;
	...
	for(var counter=1;counter<=totalRows;counter++)
	{
		...
		basicForCalculation = basicForCalculation + document.getElementById("basicForCalculation"+counter).value + "~";
...
//		          } );
	...
	.ajax({
		type : "GET",
		url : "saveSuspensionBrokenPeriodPay/" + sevaarthid + "/"+empId+"/"+fromDates+"/"+toDates+"/"+noOfDays+"/"+paybillMonth+"/"+paybillYear+"/"+basicPays+"/"+allowancesCodes+"/"+allowancesValues+"/"+deductionCodes+"/"+deductionValues+"/"+netPays+"/"+reasons+"/"+remarks+"/"+basicForCalculation+"/"+ddoCode,
		async : true,
		contentType : 'application/json',
		error : function(data) {
...
function checkEmpSuspensionDate(toDate,fromDate,sevaarthid,empId) {
var toDate = toDate;
...
var status = false ;
		if (fromDate != '') {
			...
			$
					.ajax({ 
						type : "GET",
						url : "checkEmpSuspensionDate/"+toDate+"/"+fromDate+"/"+sevaarthid+"/"+empId,
						async : false,
						contentType : 'application/json',
						error : function(data) {
							  $("#loaderMainNew").hide();
...
function checkRegPaybillAlreadyExistsforDate(toDate,fromDate,sevaarthid,empId) {
	var toDate = toDate;
	var fromDate = fromDate;
	var sevaarthid = sevaarthid;
	var empId = empId;
	...
	var status;
			if (fromDate != '') {
				...
				$
						.ajax({ 
							type : "GET",
							url : "checkRegPaybillAlreadyExistsforDate/"+toDate+"/"+fromDate+"/"+sevaarthid+"/"+empId,
							async : false,
							contentType : 'application/json',
							error : function(data) {
								  $("#loaderMainNew").hide();
							...
							success : function(data) {
								...
								if (checkFlag == 0) {
									...
									swal('Paybill is already generated for the employee !!');
									document.getElementById("txtFromDate").value = "";
									 status = false;
