...
public class DDOSchemeRepoImpl implements DDOSchemeRepo {
	...
	public List<Object[]> getAllSchemesForDDO(String lStrDDOCode) {
		...
		Session currentSession = entityManager.unwrap(Session.class);
		String hql = "select rlt.Scheme_Code,mst.scheme_Name,rlt.sub_scheme_code,rlt.Ddo_Code FROM RLT_DCPS_DDO_SCHEMES rlt,Mst_Scheme mst where mst.scheme_Code=rlt.Scheme_Code and rlt.Ddo_Code in (select zp_ddo_code from rlt_zp_ddo_map where REPT_DDO_CODE='"+lStrDDOCode+"')";
		System.out.println("HQL:"+hql);
		Query query = currentSession.createNativeQuery(hql);
		return query.getResultList();
	}
