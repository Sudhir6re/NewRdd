...
public class ZpDDOOfficeController extends BaseController {
	...
	public String approveDdoOfficeDataList(@ModelAttribute("newRegDDOModel") NewRegDDOModel newRegDDOModel, Model model,
			Locale locale, HttpSession session) {
		OrgUserMst messages = (OrgUserMst) session.getAttribute("MY_SESSION_MESSAGES");

		String message = (String) model.asMap().get("message");

		List<ZpRltDdoMap> zpDDOOfficelst = zpDDOOfficeService.getAllDDOOfficeDtlsDataByPostID(messages.getUserName());
	...
	public String updateApproveStatus(@ModelAttribute("newRegDDOModel") NewRegDDOModel newRegDDOModel,
			@PathVariable String zpDdoCode, @PathVariable int flag, Model model, Locale locale, HttpSession session,
			HttpServletRequest request, Object paybillHeadMpgRepo) {
	...
	public String getReport(@ModelAttribute("newRegDDOModel") NewRegDDOModel newRegDDOModel, Model model, Locale locale,
			HttpSession session) {
		OrgUserMst messages = (OrgUserMst) session.getAttribute("MY_SESSION_MESSAGES");
